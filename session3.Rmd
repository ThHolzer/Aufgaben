...
#  gsub("[[:punct:]]", " ", x)
```
Das funktionierte auch nicht. Bin mir zwar nicht sicher warum aber ich denke es liegt daran, dass es nicht in der Tabelle funktioniert oder so. Aus der Help funktion bin ich auch nicht schlauer geworten. Ich habe weiter auf Stckoverflow gesucht aber keine anwendbare Lösung gefunden. Wahrscheinlich gibt es eine aber ich find sie nicht. 
...
```{r}
#str_replace_all(all, "[aeiou]", toupper)
```
...
Wenn man die Minimum Länge der Verbindungen erhöht, sollte man theoretisch auch nur die angezeibt bekommen deren Bücher man mehrheitlich gut fand.
